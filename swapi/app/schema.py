...
from .models import Planet, People, Film, Director, Producer, Character
...
class AddUpdatePlanet(graphene.relay.ClientIDMutation):
    class Input:
    ...
    def mutate_and_get_payload(cls, args, context, **kwargs):
        ...
        return AddUpdatePlanet(planet=planet)
...
class FilmsInput(graphene.InputObjectType):
    """ Esto se puede utilizar dentro del class Input del AddProducer """
...
class AddCharacter(graphene.relay.ClientIDMutation):
    class Input:
        id = graphene.ID(required=False)
        name = graphene.String(required=True)
        height = graphene.String(required=False)
        ...
        birth_year = graphene.String(required=False)
        gender = graphene.String(required=False)
        films = graphene.List(of_type=FilmsInput)
    ...
    def mutate_and_get_payload(cls, args, context, **kwargs):
        ...
        films = kwargs.pop('films')

        kw = {'model': Character, 'data': kwargs}
        if raw_id:
            kw['id'] = from_global_id(raw_id)[1]
        character = generic_model_mutation_process(**kw)
        ...
        if films:
            ...
            for film_name in films:
                q |= Q(title=film_name['title'])
            films_instances = [film for film in Film.objects.filter(q)]
            character.films.set(films_instances)
...
class Query(graphene.ObjectType):
    ...
    all_character = DjangoFilterConnectionField(CharacterNode)
...
class Mutation(graphene.ObjectType):
    add_update_planet = AddUpdatePlanet.Field()  # TODO: debería tener 1 para add y otro para update?
